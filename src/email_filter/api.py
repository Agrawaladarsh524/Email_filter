...
def submit_feedback(message_id: str, req: FeedbackRequest) -> dict[str, Any]:
    """Human-in-the-loop feedback to correct an AI classification."""
    s = get_settings()
    ...
    try:
        rag_store.update_human_feedback(message_id, req.correct_category)
        
        # Also update the primary seen DB so stats are correct
        _store().mark(message_id, req.correct_category)
        
        return {"status": "success", "message_id": message_id, "updated_category": req.correct_category}
    except Exception as exc:
        raise HTTPException(500, f"feedback update failed: {exc!r}") from exc
